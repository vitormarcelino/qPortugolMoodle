...
/**
 * Test helpers for the qportugol question type.
 *
 * @package    qtype_qportugol
...
/**
 * Test helper class for the qportugol question type.
 *
 * @copyright  2013 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_qportugol_test_helper extends question_test_helper {
     ...
     * @return qtype_qportugol_question
     */
    protected function initialise_qportugol_question() {
        question_bank::load_question_definition_classes('qportugol');
        $q = new qtype_qportugol_question();
        test_question_maker::initialise_a_question($q);
        $q->name = 'QPortugol question (HTML editor)';
        ...
        $q->qtype = question_bank::get_qtype('qportugol');

        return $q;
    }

    /**
     * Makes an qportugol question using the HTML editor as input.
     * @return qtype_qportugol_question
     */
    public function make_qportugol_question_editor() {
        return $this->initialise_qportugol_question();
    }

    /**
     * Make the data what would be received from the editing form for an qportugol
     ...
     */
    public function get_qportugol_question_form_data_editor() {
        $fromform = new stdClass();

        $fromform->name = 'QPortugol question (HTML editor)';
    ...
    /**
     * Makes an qportugol question using the HTML editor allowing embedded files as
     * input, and up to three attachments.
     * @return qtype_qportugol_question
     */
    public function make_qportugol_question_editorfilepicker() {
        $q = $this->initialise_qportugol_question();
     ...
     * @return stdClass the data that would be returned by $form->get_gata();
     */
    public function get_qportugol_question_form_data_editorfilepicker() {
        $fromform = new stdClass();

        $fromform->name = 'QPortugol question with filepicker and attachments';
        $fromform->questiontext = array('text' => 'Please write a story about a frog.', 'format' => FORMAT_HTML);
        $fromform->defaultmark = 1.0;
        $fromform->generalfeedback = array('text' => 'I hope your story had a beginning, a middle and an end.', 'format' => FORMAT_HTML);
    ...
    /**
     * Makes an qportugol question using plain text input.
     * @return qtype_qportugol_question
     */
    public function make_qportugol_question_plain() {
        $q = $this->initialise_qportugol_question();
     ...
     * @return stdClass the data that would be returned by $form->get_gata();
     */
    public function get_qportugol_question_form_data_plain() {
        $fromform = new stdClass();
    ...
    /**
     * Makes an qportugol question using monospaced input.
     * @return qtype_qportugol_question
     */
    public function make_qportugol_question_monospaced() {
        $q = $this->initialise_qportugol_question();
        $q->responseformat = 'monospaced';
        return $q;
    }

    public function make_qportugol_question_responsetemplate() {
        $q = $this->initialise_qportugol_question();
    ...
    /**
     * Makes an qportugol question without an inline text editor.
     * @return qtype_qportugol_question
     */
    public function make_qportugol_question_noinline() {
        $q = $this->initialise_qportugol_question();
